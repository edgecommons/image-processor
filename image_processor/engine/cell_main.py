...
def warmup_feed(bundle_root: Path, manifest, sample: dict) -> dict:
    ...
    A sample names its inputs either as ``{"inputs": {"<tensor>": "warmup/input-01.bin"}}`` or, for
    a model with one input, as ``{"input": "warmup/input-01.bin"}``. Each entry is the member path
    of a raw tensor, or a mapping carrying ``path`` and, when the manifest declaration is not the
    right one, ``dtype`` and ``shape``.
    ...
    if isinstance(entries, str):
        ...
        entries = {manifest.inputs[0].name: entries}
